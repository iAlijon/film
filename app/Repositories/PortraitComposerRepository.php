...
use App\Models\PortraitComposer;
use App\Traits\ImageUploads;

class PortraitComposerRepository extends BaseRepository
{
    use ImageUploads;
    public function __construct()
    {
        $this->model = new PortraitComposer();
    }
    ...
    public function create($data)
    {
        $model = $this->model->create([
            ...
            'content_oz' => contentByDomDocment($data['content_oz'], 'portrait_composer'),
            'content_uz' => contentByDomDocment($data['content_uz'], 'portrait_composer'),
            'content_ru' => $data['content_ru']??null,
            'content_en' => $data['content_en']??null,
            'birth_date' => $data['birth_date'],
            'status' => $data['status'],
            'images' => $this->uploads($data['image'], 'portrait_composer')
        ]);
        if ($model)
        {
            return $model;
    ...
    public function update($data, $id)
    {
        ...
        if ($model->images)
        {
            deleteImages($model->images, 'portrait_composer');
        }
        $model->update([
            'full_name_oz' => $data['full_name_oz'],
            'full_name_uz' => $data['full_name_uz'],
