    ...
    <section class="content mb-5">
        <div class="col-11 ml-auto mr-auto">
            <div class="card card-info card-outline">
                ...
                <div class="card-body">
                    <form action="{{route('actor_conversation.update', $model->id)}}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="actor_id">
                                        ...
                                        @foreach($actors as $actor)
                                            <option value="{{$actor->id}}" {{$model->actor_id == $actor->id?'selected':''}}>{{$actor->full_name_oz}}</option>
                                        @endforeach
                                    </select>
                                </div>
